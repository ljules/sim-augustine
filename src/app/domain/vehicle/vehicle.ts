...
export class Vehicle {
  ...
  stepNoInductance(u_mot: number, alphaRad: number, v: number): { dv_dt: number; dx_dt: number; i: number } {
    ...
    // Résistance/entraînement pente & pesanteur ( forcePente = m * g * sin(alphaPenteRad) ):
    ...
    // Résultante des forces de frottements visqueux (forceVisq = fv * v ) :
    const force_visq = this.cfg.fv * v;

    // Résultante de la trainée aérodynamique (force_aero = copysign(0.5*rho*s*cx*v^2, v)) :
