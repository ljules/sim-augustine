...
@Component({
  selector: 'app-simulation-start-page',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './simulation-start-page.component.html',
  styleUrl: './simulation-start-page.component.css'
})
export class SimulationStartPageComponent {
  dt = 0.2;
  tMax = 400;
  vInitKmh = 0;
  ...
  constructor(
    ...
    this.strategy = this.normalizeStrategy(this.sessionConfig.startLapStrategy);
    this.persistStartLapStrategy(this.strategy);
  ...
  simulate(method: SolverMethod): void {
    ...
    if (!circuitProfile) {
      ...
      this.raceSessionStore.setStartLapResult(null);
    ...
    this.sessionConfig = this.raceSessionStore.get();
    const strategyCfg = this.normalizeStrategy(this.sessionConfig.startLapStrategy);
    this.persistStartLapStrategy(strategyCfg);
    ...
    try {
      ...
      this.result = res;
      this.raceSessionStore.setStartLapResult(res);
  ...
  private persistStartLapStrategy(strategy: StrategyConfig): void {
    this.sessionConfig = {
      ...this.raceSessionStore.get(),
      startLapStrategy: strategy,
    };
